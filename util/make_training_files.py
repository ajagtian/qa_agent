...
def	coarse_filter(lines):
		f = open('../data/training_data/coarse.train', 'w')
		for line in lines:
			f.write(line[:line.find(':')]+line[line.find(' '):])
		f.close()	


def	fine_filter(lines, coarse_classes):
		file_handles = {}
		for coarse_class in coarse_classes.keys():
			file_handles[coarse_class] = open('../data/training_data/fine.'+coarse_class+'.train', 'w')
...
def	main():
		args = sys.argv[1:]
		if len(args) != 1:
			print('usage: ./make_training_files.py trec_source_file')
			sys.exit(0)
